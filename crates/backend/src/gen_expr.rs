...
pub(crate) fn gen_stat(asm: &mut Assembler, frame: &mut Frame, ctx: &EmitCtx, tree: &Tree) {
    match &tree.kind {
        ...
        _ => {
            ...
            // The erased descriptor is the authority for whether a discarded
            // expression actually leaves a value. A library method such as
            // Slick's `withTransaction` is typed as `Unit` at this call site
            // but returns `Object` after erasure. The symbolic predicates
            // cannot see that fact for every classpath method, while the
            // assembler's stack model observes the exact emitted path.
            let stack_before = asm.stack;
            ...
            if is_unit_like(&tree.ty) {
                ...
                while asm.stack > stack_before {
                    match asm.stack_entries().last() {
                        Some(crate::code::StackEntry::Long)
                        | Some(crate::code::StackEntry::Double) => asm.pop2(),
                        _ => asm.pop(),
                    }
                }
            } else {
...
pub(crate) fn load_enclosing_this(asm: &mut Assembler, ctx: &EmitCtx, target: SymbolId) {
    ...
    if ctx.presuper_outer.is_none() && ctx.ctor_outer.is_none() {
        load_this(asm, ctx);
        return;
    }
...
pub(crate) fn gen_new_with(
    ...
    if !stores_outer {
        if let Some(outer) = outer_cls {
            ...
            {
                asm.aconst_null();
            } else if !ctx.in_constructor
                && ctx.outer_slot.is_none()
                && ctx.method_sym.is_none()
                && ctx.outer.is_none()
            {
