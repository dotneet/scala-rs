//! Inherited singleton results retain Scala signatures over JVM forwarders.
use std::fs;
...
#[test]
fn inherited_singleton_result_matches_scalac() {
    let p = root();
    let source = fixture("lzorigin.scala");
    let nsc = p.join("nsc");
    ...
    assert_eq!(
        expected.stdout,
        fs::read(fixture("expected/lzorigin.txt")).unwrap()
    );
    let out = p.join("ours");
    ...
    for oracle in [false, true] {
        ...
        assert!(
            !bad.status.success(),
            "accepted incompatible collection result"
        );
        assert!(
            String::from_utf8_lossy(&bad.stderr).contains(if oracle {
