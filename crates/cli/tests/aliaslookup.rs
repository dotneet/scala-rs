...
fn phantom_implicit_parameters_and_binary_self_aliases() {
    let Some(jar) = scala_library_jar() else { return };
    let Some(scalac) = scalac() else { return };
...
"#, Some("phantom\ntrue\n")),
        ("wrong-evidence", "object Main { implicitly[alib.PhantomEvidence[String]] }", None),
        ("wrong-alias", r#"
trait Handler { self: alib.SelfAliases with alib.SelfOther =>
...
"#, None),
        ...
        for ours in [false, true] {
            ...
            let (ok, diagnostic) = if ours {
                ...
                let o = Command::new(&scalac).arg("-cp").arg(&lib).arg("-d").arg(&out).arg(&src).output().unwrap();
                (o.status.success(), String::from_utf8_lossy(&o.stderr).into_owned())
            };
            assert_eq!(ok, expected.is_some(), "{label}, ours={ours}: {diagnostic}");
            if let Some(expected) = expected {
                assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), expected);
            }
        }
    }
    let _ = fs::remove_dir_all(dir);
...
"#)).unwrap();
        for ours in [false, true] {
            ...
            let (ok, diagnostic) = if ours {
                ...
                let o = Command::new(&scalac).arg("-d").arg(&out).arg(&src).output().unwrap();
                (o.status.success(), String::from_utf8_lossy(&o.stderr).into_owned())
            };
            assert_eq!(ok, accepted, "{label}, ours={ours}: {diagnostic}");
            if accepted {
                assert_eq!(run_java(&out, jar.to_str().unwrap()), "Right(1)\nLeft(bad)\n");
            }
        }
    }
    let _ = fs::remove_dir_all(dir);
...
"#)).unwrap();
        for ours in [false, true] {
            let out = dir.join(format!("{label}-{ours}"));
            fs::create_dir_all(&out).unwrap();
            let (ok, diagnostic) = if ours { compile_against(&out, &jar, &src, &lib) } else {
                let o = Command::new(&scalac).arg("-cp").arg(&lib).arg("-d").arg(&out).arg(&src).output().unwrap();
                (o.status.success(), String::from_utf8_lossy(&o.stderr).into_owned())
            };
            assert_eq!(ok, accepted, "{label}, ours={ours}: {diagnostic}");
            if accepted { assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "true\n"); }
        }
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#)).unwrap();
        for ours in [false, true] {
            let out = dir.join(format!("{label}-{ours}"));
            fs::create_dir_all(&out).unwrap();
            let (ok, diagnostic) = if ours { compile_against(&out, &jar, &src, &jar) } else {
                let o = Command::new(&scalac).arg("-d").arg(&out).arg(&src).output().unwrap();
                (o.status.success(), String::from_utf8_lossy(&o.stderr).into_owned())
            };
            assert_eq!(ok, accepted, "{label}, ours={ours}: {diagnostic}");
            if accepted { assert_eq!(run_java(&out, jar.to_str().unwrap()), "Some(hello)\nNone\nroundtrip\n"); }
        }
    }
    let _ = fs::remove_dir_all(dir);
}
...
fn binary_type_lambdas_capture_parameters_for_syntax_inference() {
    ...
    let lib = build_lib_jar(&dir);
    for (label, error_type, accepted) in [("captured", "String", true), ("wrong-capture", "Boolean", false)] {
        let src = dir.join(format!("{label}.scala"));
        fs::write(&src, format!(r#"
...
"#)).unwrap();
        for ours in [false, true] {
            ...
            assert_eq!(ok, accepted, "{label}, ours={ours}: {diagnostic}");
            if accepted { assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "done\n"); }
        }
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "a1,b2\ntrue\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "2\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "42\n1\nint!\nint!\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "List(1, 2)\n2\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "5\nSome(b)\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "bytes:2\n42\nspecific\ngeneric\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &format!("{}:{}", jar.display(), lib.display())), "(ok)\n(value)\n4.0\n6.0\n");
    }
    let _ = fs::remove_dir_all(dir);
}
...
fn native_universal_trait_accepts_value_class_clients() {
    ...
    fs::write(&producer, "package universal; trait Value extends Any { def value: Int; def next: Int = value + 1 }").unwrap();
...
"#).unwrap();
    for ours in [false, true] {
        ...
        assert!(ok, "ours={ours}: {diagnostic}");
        assert_eq!(run_java(&out, &jar.display().to_string()), "24\n72\nList(42)\nList()\nfalse\n42\n42\n42\nok\n(42,ok)\n");
        let bad = dir.join("Bad.scala");
        for code in [
            ...
            let ok = if ours { compile_against(&out, &jar, &bad, &jar).0 } else {
                Command::new(&scalac).arg("-d").arg(&out).arg(&bad).output().unwrap().status.success()
            };
