//! Late classpath alternatives pass through ordinary polymorphic call inference.
use std::{
    fs,
    path::PathBuf,
    process::Command,
...
#[test]
fn late_factory_inference_matches_scalac() {
    let root = std::env::temp_dir().join(format!(
        "scala-rs-late-factory-{}-{}",
        std::process::id(),
        SystemTime::now()
    ...
    let positive = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/late_factory_inference.scala");
