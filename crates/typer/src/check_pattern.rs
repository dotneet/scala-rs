    ...
    fn type_pattern(&mut self, pat: &mut Tree, sel_ty: &Type) {
        ...
        match &mut pat.kind {
            ...
            TreeKind::Apply { fun, args } => {
                ...
                // bound `a` to *its* type parameter -- "type mismatch; found:
                // A required: A". Harmless while `scala.util.Left` carried no
                // `CASE` flag, because the wrong class then lost to the
                // extractor arm below; giving the prelude the flag the library
                // pickles made it win.
