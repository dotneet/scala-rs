...
pub struct Typer {
    ...
    /// `val`s and `def`s that stand as statements of a *block* rather than members
    ...
    /// like a real member, because there is no accessor symbol to own it.
...
pub(crate) fn type_mentions_tparam_deep(ty: &Type, tp: SymbolId) -> bool {
    ...
    match ty {
        ...
        Type::BoundedWildcard { lo, hi } => lo
            .iter()
            .chain(hi.iter())
            .any(|bound| type_mentions_tparam_deep(bound, tp)),
        Type::Method { paramss, ret } => {
            paramss
                .iter()
                .flatten()
