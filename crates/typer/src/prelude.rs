...
fn add_any_members(st: &mut SymbolTable) {
    ...
    // genuinely generic over the explicit type argument (`x.asInstanceOf[String]`
    // must type as `String`, not `Any`); without a real type parameter here,
    // `TypeApply`'s `subst_tparams` has nothing to substitute and every
    // `asInstanceOf[T]` widens to `Any`.
