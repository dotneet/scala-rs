...
impl Typer {
    ...
    fn default_getter_apply(
        ...
        let getter_pt = Type::Method { paramss: vec![], ret: Box::new(Type::NoType) };
    ...
    fn materialize_tag(&mut self, pt: &Type, span: Span) -> Option<Tree> {
        ...
        let universe = self.universe_in_scope().or_else(|| {
            ...
            let mut tree = Tree::dummy(TreeKind::Ident { name: "_root_".into() });
            tree.span = span;
            for name in ["scala", "reflect", "runtime", "universe"] {
                tree = Tree::new(NodeId(0), span, TreeKind::Select {
                    qual: Box::new(tree), name: name.into(),
                });
            }
            let mark = self.diags.len();
