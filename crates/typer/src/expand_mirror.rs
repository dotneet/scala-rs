        ...
        for decl in &decls {
            ...
            if let Some(id) = id {
                if let Some(within) = &self.st.get(id).private_within {
                    ...
                    while !boundary.is_none() && self.st.get(boundary).name.trim_end_matches('$') != within {
                        boundary = self.st.get(boundary).owner;
                    }
                    if boundary.is_none() { return Err(qualified_access(&self.st.get(id).name, within)); }
    ...
    fn nsc_synthesizes(&mut self, cls: SymbolId, name: &str) -> Result<bool, String> {
        ...
        while let Some(p) = work.pop() {
            ...
            if !self.is_current_run_class(pid) {
                ...
                self.pickle.ensure_parents(&mut self.st, &mut self.binary, pid);
            }
