...
impl Typer {
    ...
    pub(crate) fn link_string_parents(&mut self) {
        ...
        if self.library_abi {
            if let (Some(ordered), Some(comparable)) = (
                ...
                if !self.st.get(ordered).parents.contains(&parent) {
                    self.st.get_mut(ordered).parents.push(parent);
                }
            }
        }
    }
    ...
    pub(crate) fn fill_conv_implicits(
        ...
        for (clause_index, clause) in self
            .conv_implicit_params(conv, from, &tree.ty)
            .into_iter()
            .enumerate()
        {
    ...
    pub(crate) fn warm_implicit_derivation_scopes(&mut self, wanted: &Type) {
        ...
        for id in candidates {
            ...
            for param in paramss.iter().flatten() {
                let param =
                    crate::symbol::subst_tparams_slice(&self.st.get(id).tparams, &fit.targs, param);
                if !scopes.contains(&param) {
                    scopes.push(param);
                }
            }
        }
        ...
        for &id in &cands {
            ...
            if let Type::Method { paramss, ret } = &*self.implicit_candidate_ty(id) {
                ...
                for w in wanted {
                    ...
                    let projected = match w {
                        Type::Class { sym, .. } => self.base_type_instance(ret, *sym, 0),
                        _ => None,
                    };
                    let inference_ret = projected.as_ref().unwrap_or(ret);
                    let args = self
                        .implicit_solve(id, ret, w, &[])
                        .map(|fit| fit.targs)
                        .unwrap_or_else(|| {
                            tps.iter()
                                .map(|tp| {
                                    crate::check::unify_one_precise(&self.st, *tp, inference_ret, w)
                                        .unwrap_or(Type::TypeParam(*tp))
                                })
                                .collect()
                        });
                    if args
                        .iter()
                        .zip(tps)
                        .all(|(arg, tp)| *arg == Type::TypeParam(*tp))
                        && !tps.is_empty()
                    {
                        continue;
                    }
                    for p in paramss.iter().flatten() {
                        ...
                        if crate::symbol::any_type(&p, &mut |t| {
                            matches!(
                                t,
    ...
    pub(crate) fn module_class_of_value(&self, sym: SymbolId, ty: &Type) -> Option<SymbolId> {
        ...
        match peeled {
            ...
            // singleton type. This is the shape of a path-dependent module
...
mod tests {
    ...
    fn module_class_of_value_peels_singleton_accessor() {
        ...
        typer.st.get_mut(module).ty = Type::ModuleRef(module);
        let accessor = typer.st.alloc(
            "Factory",
            typer.st.root,
            SymKind::Method,
            Flags::ACCESSOR,
