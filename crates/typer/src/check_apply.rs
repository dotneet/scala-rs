            ...
            match chosen {
                OverloadPick::Found(sym, mut param_tys, mut ret) => {
                    ...
                    if !sym.is_none() {
                        ...
                        if matches!(&fun.ty, Type::Overload(_) | Type::ModuleRef(_) | Type::Class { .. }) {
                            fun.ty = self
                                ...
                                .unwrap_or_else(|| match &recv_ty {
                                    Some(recv) => self.st.subst_as_seen_from(recv, &self.st.get(sym).ty),
                        ...
                        if a.ty.is_no_type() {
                            ...
                            let relaxed = match &p_shape {
                                        ...
                                        && mentions_tparam(&args[1], &open) =>
                                {
                                    ...
                                    Type::Class {
                                        sym: *sym,
                                        args: vec![args[0].clone(), crate::symbol::subst_tparams_slice(
                                            &open, &vec![Type::Wildcard; open.len()], &args[1])],
                                    }
                                }
                    ...
                    let ret = if params.len() != args.len()
                        && Self::application_head(fun).sym == sym
                    {
