...
impl Typer {
    ...
    fn check_overrides(&mut self, class_id: SymbolId, body: &[Tree], span: Span) {
        let names: std::collections::HashSet<String> = body.iter().filter_map(|tree| tree.name().map(str::to_owned)).collect();
        for base in crate::lin::linearize(&self.st, class_id).into_iter().skip(1) {
            if base.0 < self.st.prelude_end || self.st.is_source_class(base) { continue; }
            let ty = self.st.type_of_class(base);
            for name in &names {
                if self.st.lookup_member(base, name).iter().any(|id| self.st.get(*id).flags.contains(Flags::JAVA)) {
        ...
        while let Some(c) = returns.pop() {
            ...
            if c.0 >= self.st.prelude_end && !self.st.is_source_class(c) {
                ...
                self.pickle.complete_class_pattern_metadata(&mut self.st, &mut self.binary, c);
            }
