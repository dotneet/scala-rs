...
impl Typer {
    ...
    pub(crate) fn implicit_solve(
        ...
        let projected = match (ret, pt) {
            (
                Type::Refined { .. } | Type::ModuleRef(_) | Type::ThisType(_),
                Type::Class { sym, .. },
                        ...
                        (param.tparams.is_empty()
                            ...
                            && !tps.iter().any(|other| {
                                [&self.st.get(*other).bound_lo, &self.st.get(*other).bound_hi]
                                    ...
                                    .flatten()
                                    .any(|bound| {
                                        crate::check::type_mentions_tparam_deep(bound, *tp)
                                    })
                            }))
    ...
    fn warm_inherited_extension_members(&mut self, name: &str, span: Span) -> bool {
        ...
        while let Some(class_sym) = work.pop() {
            ...
            let external =
                class_sym.0 >= self.st.prelude_end && !self.st.is_source_class(class_sym) && {
    ...
    pub(crate) fn search_extension_in(
        ...
        let mut collect = |ids: Vec<SymbolId>| {
            ...
            for id in ids {
                ...
                if members.is_empty()
                    || (cls.0 >= self.st.prelude_end && !self.st.is_source_class(cls))
                {
                    let precise = self
        ...
        a != b
            && match (self.conversion_arg_ty(a), self.conversion_arg_ty(b)) {
                (Some(aa), Some(ab)) => {
                    ...
                    self.weak_conforms(&aa, &ab) && !self.weak_conforms(&ab, &aa)
                }
                _ => false,
            }
    }
    ...
    pub(crate) fn conversion_result(&self, id: SymbolId, from: &Type) -> Option<Type> {
        ...
        let cand_ty = self.implicit_candidate_ty(id);
        // A class that *inherits* `Function1` (`<:<`, `=:=`, a user's
        // `extends (A => B)`) is a view, so read the inherited shape when the
        // candidate is not already a function (`agent/libconc`).
        let inherited = self.conv_inherited_function1(&cand_ty);
        let cand_ty: &Type = inherited.as_ref().unwrap_or(&cand_ty);
        match cand_ty {
            Type::Method { paramss, ret } => {
    ...
    fn conv_targs_impl(
        ...
        if solve_from_implicits {
            ...
            if let (Type::Applied { ctor, .. }, Type::Method { paramss, .. }) = (param, &*cand_ty) {
                if matches!(ctor.as_ref(), Type::TypeParam(tp) if tps.contains(tp))
                    && paramss.len() > 1
                {
                    let arguments = |values: &[Option<Type>]| -> Vec<Type> {
                        tps.iter()
                            .zip(values)
                            .map(|(tp, value)| value.clone().unwrap_or(Type::TypeParam(*tp)))
                            .collect()
                    };
                    ...
                    if !witnesses_fit(&solved) {
                        ...
                        let expected =
                            self.st
                                .expand_applied_hk_alias(crate::symbol::subst_tparams_slice(
                                    tps,
                                    &arguments(&alternate),
                                    param,
                                ));
                        ...
                        if !matches!(expected, Type::TypeParam(_)) {
                            for (tp, value) in tps.iter().zip(&mut alternate) {
                                if value.is_none() {
                                    *value =
                                        crate::check::unify_one(&self.st, *tp, &expected, from);
                                }
                            }
                            let args = arguments(&alternate);
                            let expected = self.st.expand_applied_hk_alias(
                                crate::symbol::subst_tparams_slice(tps, &args, param),
                            );
