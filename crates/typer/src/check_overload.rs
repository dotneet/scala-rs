...
impl Typer {
    ...
    pub(crate) fn select_overloaded_module_apply(&mut self, fun: &mut Tree, chosen: SymbolId) {
        if !chosen.is_none() && matches!(fun.ty, Type::ModuleRef(_)) {
            ...
            fun.ty = self.st.subst_as_seen_from(&receiver.ty, &self.st.get(chosen).ty);
        ...
        match applicable.len() {
            ...
            _ => {
                ...
                if winners.is_empty() {
                    ...
                    winners = applicable.iter().filter(|a| {
                        applicable.iter().all(|b| a.0 == b.0 || weight(a, b) > weight(b, a))
                    }).cloned().collect();
                }
    ...
    pub(crate) fn complete_overload_owners(&mut self, fun: &Tree) {
        ...
        for owner in owners {
            ...
            self.pickle.ensure_parents(&mut self.st, &mut self.binary, cls);
        }
    }
    ...
    pub(crate) fn owner_is_proper_subclass(&self, a: SymbolId, b: SymbolId) -> bool {
        ...
        if let (Some((ao, _)), Some((bo, _))) = (
            ...
            if self.st.get(a).pickled_owner_bases.iter().any(|base| base == bo) {
                return true;
            }
        }
        let declaration_owner = |m: SymbolId| {
            let symbol = self.st.get(m);
            symbol.pickled_origin.split_once('#')
                .and_then(|(owner, _)| crate::classpath::find_by_jvm(&self.st, &owner.replace('.', "/")))
    ...
    pub(crate) fn proto_arg_type(
        ...
        if let Type::Overload(alts) = fun_ty {
            if let Some(p) = sole_arity_match_param(alts, idx, nargs, |alt, total| {
                self.overload_member_types.get(&sym.0)
    ...
    pub(crate) fn type_function(
        ...
        if let Some((from, _to)) = &pf_result {
            if !self.st.is_sub_type(from, &param_tys[0]) {
                return Type::Function { params: param_tys, ret: Box::new(ret) };
            }
...
fn sole_arity_match_param(alts: &[Type], idx: usize, nargs: usize, missing: impl Fn(&Type, usize) -> bool) -> Option<Type> {
    ...
    for a in alts {
        ...
        let fits = ps.len() == nargs || (repeated && nargs + 1 >= ps.len()) || (nargs < ps.len() && missing(a, ps.len()));
        if fits {
            if found.is_some() {
                return None;
            }
