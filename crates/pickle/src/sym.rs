    ...
    match t {
        SigType::Ref { sym, args } => {
            ...
            match map.get(sym) {
                ...
                Some(SigType::Poly { tparams, result })
                    if !args.is_empty() && tparams.len() == args.len() =>
                {
...
mod tests {
    ...
    fn applying_a_substituted_type_lambda_preserves_nested_binders() {
        use super::*;
        let reference = |name: &str| SigType::Ref {
            sym: name.into(),
            args: vec![],
        };
        let parameter = |name: &str| TParam {
            name: name.into(),
            bounds: SigType::Bounds {
                lo: Box::new(reference("scala.Nothing")),
                hi: Box::new(reference("scala.Any")),
            },
            variance: 0,
        };
        let lambda = SigType::Poly {
            tparams: vec![parameter("A")],
            result: Box::new(SigType::Poly {
                tparams: vec![parameter("B")],
                result: Box::new(SigType::Ref {
                    sym: "Pair".into(),
                    args: vec![reference("A"), reference("B")],
                }),
            }),
        };
        let input = SigType::Ref {
            sym: "F".into(),
            args: vec![reference("B")],
        };
        let output = apply_subst(&input, &HashMap::from([("F".into(), lambda)]));
        let SigType::Poly { tparams, result } = output else {
            panic!("lost nested binder")
        };
        ...
        assert_eq!(
            *result,
            SigType::Ref {
                sym: "Pair".into(),
                args: vec![reference("B"), reference(&tparams[0].name)]
            }
        );
    }
