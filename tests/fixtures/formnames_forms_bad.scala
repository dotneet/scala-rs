...
object Main {
 def form = mapping("url" -> text(), "events" -> events, "ctype" -> text(), "token" -> optional(text()))((url:String,events:Int,ctype:String,token:Option[String]) => (url,events,ctype,token))
 def events: ValueType[Set[String]] = new ValueType[Set[String]] {
