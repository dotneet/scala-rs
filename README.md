...
| `array_ops7.scala` | `doubleArrayOps` / `booleanArrayOps` の `head` / `map`（library dual-run のみ。`unitArrayOps` wrapper はあるが Unit 配列の dual-run はしない。char/float/byte/short/int/long/ref は触らない） | `1.0` `2.0` `3.0` `true` `false` `true` |
...
- 残りの **StringOps**（`partition` / `exists` / `forall` / `splitAt` / `updated` / `count` / `span` / `diff` / `intersect` / `split(String)` / `filter` / `reverseIterator` 以外）
- 残りの **ArrayOps**（primitive wrappers の `head`/`map` は揃った。他メソッド。`genericArrayOps`）
- 他の mutable（`ArrayDeque` / `LinkedHashMap` / `LinkedHashSet` / `HashMap` / `HashSet` / `ArrayBuffer` / `ListBuffer` 以外）
