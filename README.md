...
- 残りの **StringOps**（`filter` / `reverseIterator` 以外。`padTo` 以降に無いもの、`*:` など）
- 残りの **ArrayOps**（`map` + `ClassTag` はこのスライスで載せた。他 primitive wrapper `byteArrayOps` 等）
