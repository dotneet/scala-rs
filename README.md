...
- XML リテラルのサブセット（2.13）: `<a>t{e}</a>` / `<a/>` / `<a b={e} c="t"/>`（elem / text / splice / 非プレフィックス属性）。属性は nsc と同じ `UnprefixedAttribute(key, value, next)` チェーン。名前空間は診断する（黙って捨てない）。`scala-rs run` は検出できた scala-xml jar を `java -cp` に足す
