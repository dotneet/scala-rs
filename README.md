...
- パラメータ、ラムダ（型付き / 期待型から推論）、ブロック。**placeholder `_`**（nsc `withPlaceholders`）: `_ + 1` / `_.abs` / `f(_)` / `xs.map(_ + 1)` を合成 `Function` にする。bare `_` は `unbound placeholder parameter`。パターン / 存在型 / デフォルト引数の `_` とは別機能
...
- 残りの **StringOps**（`diff` / `intersect` / `split(String)` 以外。`updated` / `count` / `span` など）
- 残りの **ArrayOps**（`byteArrayOps` / `shortArrayOps` の `head`/`map` はこのスライス。他 primitive wrapper とメソッド）
...
- placeholder の残り（入れ子セクションの完全再現、`_ + _` の Function2 をこのスライスの必要形以外まで広げない）
