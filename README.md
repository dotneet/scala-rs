...
- 残りの **ArrayOps**（primitive wrappers と `genericArrayOps` の `head`/`map` は揃った。他メソッド）
- 他の mutable（`ArrayDeque` / `LinkedHashMap` / `LinkedHashSet` / `HashMap` / `HashSet` / `ArrayBuffer` / `ListBuffer` 以外）と他の immutable（`SortedSet` / `TreeSet` / `Set` / `Map` / `Vector` 以外）
