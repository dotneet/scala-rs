...
- **leftover pickle holes**（nsc 完全 pickle ではない）: 非リテラルのアノテーション引数（TREE）、残る Flags ビット（Java 由来メンバーの JAVA、MACRO / BRIDGE / VARARGS / late・anti flags など）。Java `@Deprecated` の SYMANNOT、入れ子 `List[_ <: List[_]]`、refinement `A with B { def f: Int }`（REFINEDtpe）はこのスライスで載せた。full pickle とは主張しない
