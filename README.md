...
- クラス / トレイトの nullary 境界付き型メンバー `type A <: T`（refinement の `{ type A <: T }` と HK `type F[_] <: Bound` は実装済み）
