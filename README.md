...
- SIP-23 定数型のサブセット: `val x: 1 = 1`、`def f(x: 1): Int`。式のリテラルは定数型（`1 <: Int`）。不一致 `val y: 1 = 2` は type mismatch。pickle の CONSTANTINTtpe は出さない（Remaining の leftover pickle holes）
- `scala.Dynamic`: `d.foo` → `selectDynamic("foo")`、`d.foo(args)` → `applyDynamic("foo")(args)`。`import scala.language.dynamics` が必要。`--scala-library` 時は jar の `scala/Dynamic` に対して実行する
