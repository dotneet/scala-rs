...
- パラメータ、ラムダ（型付き / 期待型から推論）、ブロック。**placeholder `_`**（nsc `withPlaceholders`）: `_ + 1` / `_.abs` / `f(_)` / `xs.map(_ + 1)` に加え **`_ + _` を Function2**、入れ子セクション `_.map(_ + 1)`（内側の非 bare `_` を先に wrap）。期待型と arity が合わない `_ + _` は nsc と同じ `missing parameter type for expanded function`。bare `_` は `unbound placeholder parameter`。パターン / 存在型 / デフォルト引数の `_` とは別機能
...
- 残りの **StringOps**（`updated` / `count` / `span` / `diff` / `intersect` / `split(String)` / `filter` / `reverseIterator` 以外。`partition` / `exists` / `forall` / `splitAt` など）
- 残りの **ArrayOps**（`charArrayOps` / `floatArrayOps` の `head`/`map` はこのスライス。`doubleArrayOps` / `booleanArrayOps` / `unitArrayOps` と他メソッド）
...
- placeholder の残り（より深い入れ子・typed `_ : T` の完全再現。unary `_ + 1` と Function2 `_ + _` / `_.map(_ + 1)` はこのスライス）
