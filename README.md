...
- 残りの **StringOps**（`tail` / `init` / `distinct` / `mkString` など。このスライスの `head` / `last` / `stripLineEnd` / `replaceAllLiterally` 以外）
- 残りの **ArrayOps**（`map` + `ClassTag`、`foreach`、他 primitive の `longArrayOps` 等。このスライスは `intArrayOps` の `head` / `tail` のみ）
- 他の mutable（`LinkedHashSet` など）。`HashMap` / `HashSet` は再実装しない
