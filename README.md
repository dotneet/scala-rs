...
- refinement の高階型メンバー `{ type F[_] }` / 境界付き型メンバー `type A <: T`（subclass の `type F[X] = Id[X]` とパス依存 `m.F[Int]` は実装済み）
