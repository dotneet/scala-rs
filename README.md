...
| `array_ops3.scala` | `intArrayOps` の `foreach` と `longArrayOps` の `head` / `foreach`（library dual-run のみ。`map` + ClassTag は未リンク） | `1` `2` `3` `10` `10` `20` `30` |
...
- 残りの **StringOps**（`padTo` 以降に無いもの、`*:` など。このスライスの `tail` / `init` / `distinct` / `mkString` 以外）
- 残りの **ArrayOps**（`map` + `ClassTag` は ClassTag の第二パラメータ節をこのスライスでは載せなかった。他 primitive wrapper `refArrayOps` / `byteArrayOps` 等）
- 他の mutable（`LinkedHashMap` / `LinkedHashSet` / `HashMap` / `HashSet` 以外）
