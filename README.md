...
- コンテキストバウンド `T: ClassTag` / `T: Ordering` / `T: scala.reflect.ClassTag`（メソッド型パラメータ）。nsc と同様、implicit evidence `C[T]` へデシュガーする。クラス型パラメータの `: C` は診断する。evidence が無ければ `no implicit`。`--scala-library` 時は `Ordering[Int]` と `ClassTag` が jar にリンクして動く
- `lazy val`
