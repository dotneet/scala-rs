...
### `<:<` / `=:=`（型制約）と `Option.orNull`（`--scala-library` 専用）

- `scala.<:<[-From, +To]`（JVM `scala/$less$colon$less`、`Function1[From, To]` を extends）と `scala.=:=[From, To]`（JVM `scala/$eq$colon$eq`、`<:<[From, To]` を extends）。中置記法 `A <:< B` / `A =:= B` は型位置でそのまま書ける（`Either` などと同じ、汎用の中置型サポート経由）。`def f[A, B](x: A)(implicit ev: A <:< B): B = ev(x)` が動く
- 実 2.13.16 の jar には `scala/$eq$colon$eq$`（`=:=` のコンパニオン）classfile が存在せず、汎用の witness は `<:<` のコンパニオンにある `refl[A]: A =:= A` だけ。`Predef.$conforms[A]` も実際は `Function1[A, A]` を返す（`<:<[A, A]` ではない）ため、そのままでは `A <:< B` の witness として使えない ― 実 scalac 同様
- このコンパイラの `search_implicit` は、それ自身の型パラメータを持つ多相な implicit def（`refl[A]` のような）を候補としてユニファイしない。そのため `A <:< B` / `A =:= B` の implicit 探索は、`crates/typer/src/check.rs` の専用フォールバック `conforms_witness_fallback` が `is_sub_type(A, B)` を直接確認し、成り立てば `<:<.refl[A]()` を witness として合成する（成り立たなければ通常どおり「implicit が見つからない」診断）
- `Option[A].orNull(implicit ev: Null <:< A): A`。nsc の実シグネチャは `orNull[A1 >: A]` と新しい型パラメータを導入するが、こちらは `Option` 自身の `A` をそのまま再利用する簡略版（`Option[Int].orNull` のように `Null <:< A` が成り立たないケースは実 scalac 同様にコンパイルエラーになる）
- `Iterable` / `IterableOnce` に `foreach` を追加し、`List <: Iterable` を明示（`List <: IterableOnce` の既存配線と同じ形）
- 私有ランタイムには対応する classfile がないため `library_abi`（`--scala-library`）限定。`--no-scala-library` では `<:<` / `=:=` はそもそもインストールされず、`not found: type <:<` のような通常の診断になる
...
| `conformty.scala` | `A <:< B` / `A =:= B` の中置型と implicit witness（`ev(x)`）、`Option.orNull`、`Iterable.foreach`（library dual-run のみ） | `42` `hello` `present` `null` `10` |
...
- `<:<` / `=:=` は `refl` のみ（`=:=.tpEquals` は実 2.13.16 jar に対応する companion classfile が存在せず未実装。`<:<.antisymm` も未）。`Option.orNull` は `Option` 自身の型パラメータを再利用する簡略版で、nsc の `orNull[A1 >: A]`（別の nullable 型への upcast）は未対応。`List[(K, V)].toMap` のような、witness 自身が独自の型パラメータを持つ形の implicit 解決一般（`search_implicit` が多相 implicit def をユニファイしない制限）はこのスライスでは直さず、`conforms_witness_fallback`（`crates/typer/src/check.rs`）による `<:<` / `=:=` 専用の特別扱いにとどめた
