...
| `anoncap2.scala` | キャプチャ + `$outer`（囲みクラスのメンバと同時参照）、匿名クラス内のラムダによる二重キャプチャ、入れ子匿名クラス、ラムダの中の `new`、lambda-lift されるネスト `def` の中の `new`、trait のメソッドの中の匿名クラス（`$outer` はインタフェース型）（両 ABI で実行） | `holder 15` `14` `inner 42` `16` `12` `106` |
