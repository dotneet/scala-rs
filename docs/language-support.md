...
- **Named arguments** (reordered at the call site). They work for methods, for a companion's `apply`, for a case class's `copy`, for **constructors `new C(b = 2, a = 1)`**, for **overloaded calls**, and with varargs (`f(a = 1)` / `f(a = 1, 2, 3)` for `def f(a: Int, rest: Int*)`). Reordering follows the same rules as nsc's `NamesDefaults.removeNames`: **a named argument that is in its own position permits positional arguments after it** (`f(a = 1, 2)` is accepted, while `f(b = 1, 2)` is `positional after named argument.`). As in nsc, overloads are first narrowed by parameter **name**, and the argument types decide only when names alone are not enough. The diagnostics use exactly real scalac's wording (`unknown parameter name: q` / `parameter 'c' is already specified at parameter position 2` / `positional after named argument.`) and, as in nsc, only one is emitted per call (follow-on messages such as "not enough arguments" are cascades and are suppressed)
- Mixin of traits with concrete members (`T$class` static implementations + forwarders in linearisation order). Forwarders are emitted for both `class` and `object`. The runtime representation of a trait's `val` / `override val` / `var` is covered in the "Trait mixin" section
...
- `super` and qualified `this` (`Outer.this`). A trait's `super` goes through `T$class` for a concrete class, or through `T$$super$m` for a stackable `abstract override`
...
- Initialisation of traits with concrete `val`s (`T$class.$init$`) and the `super` chain of `abstract override`
...
writes them out as static methods. So that nested class emission (anonymous classes, a trait's `$class`)
...
Java 6 has no default methods, so a trait with concrete members is emitted like this.

- The trait itself becomes a JVM interface in which every method is abstract
- Concrete bodies become static methods on `T$class` (whose first parameter is `$this: T`)
- The implementing class emits forwarders to whichever definition won linearization (the rightmost mixin is the more specific one)
...
A trait's `val` is represented by a getter on the interface plus a **mixin setter `T$_setter_$v_$eq` with the same name nsc uses** (`p$q$T$_setter_$v_$eq` when a package is involved); `T$class.$init$` evaluates the right-hand side and calls that setter. The implementing class holds the field, and its constructor calls the mixin `$init$` methods (from the more general parent onwards). `object O extends T` works the same way, emitting exactly as many fields, accessors and `$init$` calls as a class does.
...
For `abstract override` in a stackable trait, `super.m` inside `T$class` becomes `T$$super$m` (the implementing class forwards to the next entry in the linearization). With `class C extends Base with A with B` where both declare `abstract override def msg`, the runtime result is `B-A-base`.
...
`Main$Loud` also has `java/lang/Object` as its super). So when a `T$class` body reads an
...
- A **trait nested in a class** (`class Outer { trait T { def d = v } }`) cannot hold a field on
  ...
  (`Main$Outer$T$class`) calls that accessor rather than doing `getfield $outer`.
...
`super.m(...)` is `invokespecial` for a class parent, and `T$class.m($this, ...)` for a concrete
