...
- `s"..."` / `f"..."` / `raw"..."` string interpolation. `f"$n%02d"` is lowered to `String.format`. `raw` does not interpret escapes. Date/time (`%t`/`%T`), argument indices and the relative `% <` are reported as diagnostics. Under `--scala-library`, custom interpolators (the `q"a$x"` of `implicit class Q(sc: StringContext) { def q(args: Any*) }`) are desugared to `StringContext.apply(parts*).q(args*)` and run. On the private runtime, anything other than `s`/`f`/`raw` is a diagnostic. In a pattern (`case s"$a-$b" =>`, `val s"k=$v" = line`) an interpolation is nsc's extractor `StringContext(parts*).id(pats*)`: `s` uses the library's `StringContext#s.unapplySeq` (glob matching), and a user interpolator any parameterless member with an `unapply` / `unapplySeq`; `$_` is a pattern-only hole
...
- Variance: `class C[+A]` / `class Box[+A](val x: A)` are legal. `class Bad[+A](var x: A)` is rejected as covariant-in-contravariant, as in nsc. `A @uncheckedVariance` (in a method parameter or type argument position) turns off the variance check for that occurrence, as in nsc; a renaming import of the annotation (`import scala.annotation.unchecked.{uncheckedVariance => uv}`, slick's `HCons`) is recognised. **A type member's own parameters** are validated the way nsc's `validateVariance` does (`crates/typer/src/kind_bounds.rs`, `agent/kindvar`): a type lambda `({ type l[-a] = List[a] })#l` wherever it is written, a named higher-kinded alias `type l[-a] = Cov[a]`, and an abstract member's bounds (`type G[-x] <: List[x]`; a member's own parameter in its *lower* bound keeps its position, so `type G[+x] >: F[x]` is legal) report `contravariant type a occurs in covariant position in type [-a]List[a] of value <local l>` / `of type l` (`neg/t7872`, `neg/t7872b`). The class's parameters are invariant in a member alias's right-hand side and flipped in a lower bound (`class G[+T] { type V = List[T] }` is rejected); a definition local to a block and a `private[this]` member are exempt, as in nsc
- **Kinds of higher-kinded type arguments** (nsc `checkKindBounds`, `crates/typer/src/kind_bounds.rs`, `agent/kindvar`): a type argument passed to a higher-kinded parameter must have parameters of the same number, compatible variance and no stricter bounds, level by level -- `List` is not an `F[-_]`, `Set` is not an `F[+_]`, `class Bnd[A <: AnyVal]` is not an `F[_]`, `M[_[_]]` accepts `Functor[F[_]]` and refuses `CFunctor[F[+_]]` while `M[_[+_]]` accepts both; `Any` and `Nothing` are kind-overloaded. Checked for written method type arguments, inferred ones (`inferred kinds of the type arguments (List) do not conform to the expected kinds of the type parameters (type F).` followed by nsc's explanation lines, `neg/t7872c`) and the arguments of a written class type (`new Foo[Set]`, `… (type F) in class Foo.`); a kind error replaces the bounds error of that site. For a *class* type application only arity and variance are compared, never the bounds. scalac 2.13.16 does compare them for a plain `type f = curry[C]` (`class C[z <: NAT]`, `trait curry[n[_]]`: "type z's bounds <: NAT are stricter than type _'s declared bounds"), but whether it does depends on where the application sits in its trees rather than on the language: the same `curry[C]` inside a method body with top-level classes passes, and so does `pos/t2994a`'s `curry[m#a, s]`, where the application is an argument of an abstract higher-kinded member's application and its bounds are never visited. Rather than copy that plumbing, this compiler leaves class-application bounds unchecked; a method's type arguments (`gm[C]`) are checked the way nsc does. `FunctionN` is `[-T1, …, -Tn, +R]` for this check (`prelude_variance.rs`), and a class discovered on `-cp` has its pickle adopted before its variances are read (a separately compiled `class X[+A]` was invariant to the variance checks until something else completed it, `pos/t8708`). A bound that still mentions a type parameter after instantiation is not judged, so `def b[F[_ <: Int]]` applied to `class Cmp[A <: Comparable[A]]` is accepted where nsc reports stricter bounds
