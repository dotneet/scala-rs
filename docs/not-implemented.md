...
- **A constructor default in a *later parameter clause*.** `class Curr(a: Int)(b: String = "b" + a)` called as `new Curr(7)()` emits an `invokespecial` with one argument for a two-parameter descriptor: `VerifyError: Bad type on operand stack`. A `new`'s arguments reach `fill_defaults_and_implicits` already flattened (the typer flattens a constructor's clauses), while that function re-reads the callee's *unflattened* `paramss` off the symbol, so the second clause is never seen as short and nothing is filled. It is not about secondary constructors — it reproduces on the primary, with or without a companion — and the identical `def m(a: Int)(b: String = "b" + a)` is filled correctly. This is also the only shape in which a constructor default may legally name an earlier parameter, since nsc rejects a same-clause reference outright. Pinned by `crates/cli/tests/secondaryctor.rs`'s `a_default_in_a_later_ctor_clause_is_not_filled`
- **Preferring the constructor alternative that needs no default.** nsc fills a default only when no alternative applies without one, so `new Prefer(1)` on `class Prefer(n: Int) { def this(k: Int, bump: Int = 5) = this(k * 100 + bump) }` is the *primary*. Both alternatives are applicable here at once and the call is `ambiguous overload for constructor` — a rejection rather than a wrong pick. Pinned by `crates/cli/tests/secondaryctor.rs`'s `nsc_prefers_the_alternative_that_needs_no_default`
- **`private[p]` / `protected[p]` on a constructor read back out of a class file.** nsc pickles a qualified access as the bare `PRIVATE` (or `PROTECTED`) flag *plus* a `privateWithin` reference. `crates/pickle` records that the reference is there (`Member::private_within`) but does not resolve `p`, and `PickleSupply::install_ctor` deliberately leaves such a constructor **accessible** rather than guess: treating it as a plain `private` would refuse every `private[slick]` constructor slick's own code calls. So `new libp.Qual("x")` from another package compiles where scalac reports `constructor Qual in class Qual cannot be accessed`. Plain `private` and `protected` do survive the round trip (`agent/ctorgaps`, `neg/t6601`); pinned by `crates/cli/tests/ctorgaps.rs`'s `a_qualified_private_constructor_is_still_accepted_from_outside`
