...
- **A default argument on a *secondary* constructor.** `class Deft(val p: Int, val q: String) { def this(p: String, q: String = "dq") = this(p.length, q) }` called as `new Deft("abc")` reports `value <init>$default$2 is not a member of <the enclosing object>`. `Typer::synthesize_ctor_default_getters` is only ever handed the *primary* constructor's parameter list, so the `$lessinit$greater$default$N` the call site needs is never declared and the lookup falls out to the enclosing scope. nsc puts it on the companion module (with a static forwarder on the class) exactly as it does for a primary's default, so the naming is already right — what is missing is running the synthesis over the secondary constructors too. Found by `agent/secondaryctor` while fixing the `new`-on-a-secondary miscompile; it is a hard error rather than a silent one, and `crates/cli/tests/secondaryctor.rs` asserts the rejection so the test fails when this closes
