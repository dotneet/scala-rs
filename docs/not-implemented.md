...
- **Constructor default interoperability:** ordinary classes now synthesize the companion needed by separately compiled callers, including first-clause and auxiliary-constructor defaults. The bidirectional `sqlstoragebatch` fixture covers generic and nested classes, explicit companions and side-effecting defaults. Deeper dependent default types remain subject to the compiler's general type-system limitations.
