...
- **Default constructor arguments across a compilation run.** nsc gives a class with them a synthetic companion holding `$lessinit$greater$default$N`, and marks the parameter `DEFAULTPARAM` in the pickle. We do neither: the typer inlines a constructor default at the *call site* (`Typer::type_default_rhs_here` says so outright — "a primary constructor's defaults have no `name$default$n` getters"). `new C(1)` therefore works inside one run and fails outside it — real scalac reading our `class Box(val a: Int, val b: Int = 7)` says `not enough arguments for constructor Box: (a: Int, b: Int)`, with no diagnostic from us at the point the classfile is written. This is 12 of the 17 remaining classfiles scalac writes for slick and we do not; see [docs/notes/companions-and-class-symbols.md](notes/companions-and-class-symbols.md)
