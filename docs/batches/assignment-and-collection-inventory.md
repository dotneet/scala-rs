...
| Cats tuple swap diagnostics | Two full cats errors compare `Tuple2[C,B]` and structural `(C,B)`, but direct and function-valued tuple probes already agree with scalac. | Existing subtype/unification rules intentionally equate tuple sugar and `Tuple2`; a blanket prelude change is unsupported and deferred. | unresolved, deferred |
