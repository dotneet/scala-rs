# Remaining-family inventory during frozen 287ec595 gate

No compiler edits are allowed in the gated tree. These are hypotheses, not
measured fixes. Correcting a hypothesis through real scalac 2.13.16 probes is
more valuable than implementing the proposed diagnosis. No subagents.

Current early measures are GB92/43, cats34/23, library383/107. Candidate-specific
...
The frozen 287ec595 full gate has now exposed two existing quasi suite
failures: qq_ctx_bad_names_every_form_it_cannot_build and
qr_forms_bad_names_every_form_it_cannot_build. Both lose the explicit by-name
type refusal after the parser marker changed to <ByName>. reify.rs:1008 still
keys its refusal on the former representation. The quasi target was omitted
when prerequisite scope expanded to parser/reification. Include it and audit
all consumers of the parser marker, preserving actual scalac rejection of
standalone tq"=> Int" and runtime behavior for accepted nested forms. Do not
weaken either existing diagnostic test. Await DONE before any source change.

The marker audit now has three real compiler/runtime comparisons (marker-probe/):
1. A user class named `<ByName>`[A] prints7 with scalac and accepted-before,
   but 287ec595 refuses it: the new parser marker collides with a written name.
2. Standalone tq"=> Int" is rejected by scalac and before; 287ec595 accepts and
   executes, printing the bogus `$lessByName$greater[Int]` tree. This is a real
   false acceptance, not merely changed error text.
3. tq"(=> Int) => Int" is accepted and prints the source form with scalac; both
   before and candidate refuse it. Keep this distinct from standalone syntax.
No source changed during the gate. Fix marker identity and quasiquote syntax
boundaries together, add the positive written-name regression, preserve both
existing diagnostic tests, and inspect kind-projector marker consumers.
